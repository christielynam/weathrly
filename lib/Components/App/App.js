...
export default class App extends React.Component {
  ...
  getWeather() {
    ...
    .catch(() => { alert('Please enter a valid City and State'); });
  }
  ...
  getFromLocal() {
    ...
    if (location) {
      this.setState({ location }, this.getWeather);
    }
  }
  ...
  render() {
    const hourCards = (this.state.weatherData.hourlyForecast || []).map((hour, index) => {
      if (index <= 6) {
        return <HourCard img={hour.icon} temp={hour.temp.english} time={hour.FCTTIME.civil} key={hour.FCTTIME.hour} />;
      }
    });

    const dayCards = (this.state.weatherData.dailyForecast || []).map(day => {
      return <DayCard img={day.icon} high={day.high.fahrenheit} low={day.low.fahrenheit} day={day.date.weekday} key={day.period} />;
    }
  );
